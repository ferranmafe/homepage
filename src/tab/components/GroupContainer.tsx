...
export const GroupContainer = ({ group }: Props) => {
  ...
  const handleDeleteClick = (originalIndex: number) => {
    chrome.storage.local.get([LINKS_KEY]).then((result: LinksList) => {
      ...
      const updatedLinks = links.filter(
        (_, linkIndex: number) => linkIndex !== originalIndex
      );
  ...
  const handleSave = async (
    ...
    const result = (await chrome.storage.local.get([LINKS_KEY])) as LinksList;
    const links = result[LINKS_KEY];
    const updatedLinks = [...links];
    if (updatedLinks[originalIndex]) {
      updatedLinks[originalIndex] = { title, url, group: groupName };
      await chrome.storage.local.set({ [LINKS_KEY]: updatedLinks });
    }
  };
        ...
        <div className="p-2">
          {items.map((item: Item, index: number) => (
            <div key={item.title + index}>
              <div className="flex items-center text-lg bg-emerald-200 p-3 border rounded-lg hover:bg-emerald-500 hover:cursor-pointer group">
                ...
                <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  <FaTrash
                    ...
                    onClick={() => handleDeleteClick(item.originalIndex)}
                    aria-label="delete"
                  />
                </div>
              </div>
